...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null = null) {
  ...
  if (errorMessage.includes('Quota limit exceeded') || errorMessage.includes('quota exceeded')) {
    console.warn('Firestore Quota Exceeded. The app will operate in a limited mode until the quota resets.');
    // We don't throw a scary JSON error for quota issues, just log it
    return;
  }
